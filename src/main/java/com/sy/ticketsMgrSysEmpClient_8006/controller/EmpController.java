...
//	}
	...
	public  String showEmps(Integer orderIndex,String flag,String condition,
			Integer pageNum,Integer pageCount,ModelMap mm,Locale locale){
		if(orderIndex == null) {
			orderIndex = 1;
		}
		if(flag == null) {
		...
		//设置回显信息
		//排序列
		mm.addAttribute("orderIndex",orderIndex);
		mm.addAttribute("orderFlag",flag);
		mm.addAttribute("condition",condition);
		
		//获取分页对象
		mm.addAttribute("pageVO",
				empService.showEmps(orderIndex, flag, condition, pageNum, pageCount, locale));
		return "index";
	}
	/**
	 * 排序列名
